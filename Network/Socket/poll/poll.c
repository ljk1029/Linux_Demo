...
#define MAX_CONNECT_CNT_POLL MAX_CONNECT_CNT
#else
#define MAX_CONNECT_CNT_POLL (5)
#endif

static int run_flag = 1;


static int getAddress(int sockfd, int port, int localfd, struct sockaddr_in client_sockaddr)
{
...
static int tcpServer(int port) 
{
    ...
    // poll, 也可建立线程将检测单独出去
    struct pollfd pfd[MAX_CONNECT_CNT_POLL];
    pfd[0].fd = server_fd;
    pfd[0].events = POLLIN;
    int max_fd = server_fd;
    for (int i = 1; i < MAX_CONNECT_CNT_POLL; i++){
        pfd[i].fd = -1;
    }
    ...
    while (run_flag) 
    {
        ...
        if(pfd[0].revents & POLLIN)
        {
            ...
            // 获取连接地址和本地地址
            getAddress(client_fd, port, server_fd, address);
            ...
            // printf("max_fd %d\n",max_fd);
            // for (int i = 0; i < MAX_CONNECT_CNT_POLL; i++){
            //    printf("fd %d\n",pfd[i].fd);
            // }
        }
