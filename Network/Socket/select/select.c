...
#else
#define MAX_CONNECT_CNT_SELECT (5)
#endif
...
static int tcpServer(int port) 
{
    ...
    int max_fd = server_fd, client_s_fd[MAX_CONNECT_CNT_SELECT];

    for (int i = 0; i < MAX_CONNECT_CNT_SELECT; i++) {
        client_s_fd[i] = -1;
    }
            ...
            if (fd != -1 && FD_ISSET(fd, &r_fds)) {
                ...
                if (numbytes <= 0) 
                {
                    ...
                    if (fd == max_fd)
                    {
                        client_s_fd[i] = -1;
                        max_fd = server_fd;
                        for (int i = 1; i < MAX_CONNECT_CNT_SELECT; i++){
